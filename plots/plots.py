...
# barplot of effect_cat by ss_group
fig, ax = plt.subplots(figsize=(20, 10))
#position_scores = position_scores.merge(metadata[['resi_mut', 'ss_domain']], on=['resi_mut'], how='left')

position_scores = position_scores.loc[position_scores.resi_struct.notna(), :]
position_scores = position_scores.drop_duplicates(subset=['resi_struct'])
barplot_df = position_scores.groupby('ss_group_simple')['effect_cat'].value_counts(normalize=True).reset_index(name='proportion')
sns.barplot(x='ss_group_simple', y='proportion', data=barplot_df, ax=ax, hue='effect_cat')
plt.title('Effect Cat by SS Group Simple')
plt.savefig(f'{plot_dir}/effect_cat_by_ss_group_simple_barplot.pdf')
plt.close()
